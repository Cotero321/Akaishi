...
public interface PacketTransformer {
    void inbound(NetworkManager.Side side, class_2960 id, class_2540 buf, NetworkManager.PacketContext context, TransformationSink sink);
    
    void outbound(NetworkManager.Side side, class_2960 id, class_2540 buf, TransformationSink sink);
    
    @FunctionalInterface
    interface TransformationSink {
        void accept(NetworkManager.Side side, class_2960 id, class_2540 buf);
    }
    
    static PacketTransformer none() {
        return new PacketTransformer() {
            @Override
            public void inbound(NetworkManager.Side side, class_2960 id, class_2540 buf, NetworkManager.PacketContext context, TransformationSink sink) {
                sink.accept(side, id, buf);
            }
            
            @Override
            public void outbound(NetworkManager.Side side, class_2960 id, class_2540 buf, TransformationSink sink) {
                sink.accept(side, id, buf);
            }
        };
    }
    ...
    static PacketTransformer concat(Iterable<? extends PacketTransformer> transformers) {
        ...
        return new PacketTransformer() {
            ...
            private void traverse(NetworkManager.Side side, class_2960 id, class_2540 buf, @Nullable NetworkManager.PacketContext context, TransformationSink outerSink, boolean inbound, int index) {
