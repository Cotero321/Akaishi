...
public class SplitPacketTransformer implements PacketTransformer {
    ...
    public SplitPacketTransformer() {
        PlayerEvent.PLAYER_QUIT.register(player -> {
            cache.keySet().removeIf(key -> Objects.equals(key.playerUUID, player.method_5667()));
        });
        ...
        switch (buf.readByte()) {
            ...
            case END:
                ...
                if (data.parts.size() != data.partsExpected) {
                    ...
                    class_2540 byteBuf = new class_2540(Unpooled.wrappedBuffer(data.parts.toArray(new ByteBuf[0])));
    ...
    public void outbound(NetworkManager.Side side, class_2960 id, class_2540 buf, TransformationSink sink) {
        ...
        if (buf.readableBytes() <= maxSize) {
            ...
            class_2540 packetBuffer = new class_2540(Unpooled.wrappedBuffer(stateBuf, buf));
