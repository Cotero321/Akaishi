 ...
 * 配方数据直接复用 {@link AkaishiEnergyLiquefierBlockEntity#recipeFor}，保证与机器逻辑一致。
...
public class LiquefactionRecipeCategory implements IRecipeCategory<LiquefactionRecipeCategory.LiquefactionRecipe> {
    ...
    public void draw(LiquefactionRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
        ...
        guiGraphics.drawString(Minecraft.getInstance().font,
                Component.translatable("jei.akaishi.cost_liquefy", fmt(recipe.cost())), 10, 74, 0xFFFFFFFF);
    }
    ...
    public record LiquefactionRecipe(ItemStack input, boolean needsSolid, long cost, long amount, Fluid output) {
        ...
        public static List<LiquefactionRecipe> getAll() {
            return List.of(
                    from(AkaishiEnergyLiquefierBlockEntity.recipeFor(new ItemStack(Items.NETHER_STAR))),
                    from(AkaishiEnergyLiquefierBlockEntity.recipeFor(new ItemStack(Items.WITHER_ROSE))),
                    from(AkaishiEnergyLiquefierBlockEntity.recipeFor(new ItemStack(ModItems.endMixture.get()))),
                    from(AkaishiEnergyLiquefierBlockEntity.recipeFor(new ItemStack(ModItems.dragonMixture.get()))),
                    from(AkaishiEnergyLiquefierBlockEntity.recipeFor(new ItemStack(ModItems.sculkLifeform.get()))));
        }

        private static LiquefactionRecipe from(AkaishiEnergyLiquefierBlockEntity.Recipe r) {
