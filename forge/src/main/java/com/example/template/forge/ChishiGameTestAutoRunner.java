...
/**
 * dev 环境燃料系统端到端自动测试（以 -Dchishi.gametest=1 启动服务端触发）。
 ...
 */
public final class ChishiGameTestAutoRunner {

    private static final Logger LOGGER = LoggerFactory.getLogger(ChishiGameTestAutoRunner.class);
    ...
    private ChishiGameTestAutoRunner() {
    }
    ...
    public static void start(MinecraftServer srv) {
        ...
        if (level == null) {
            LOGGER.error("[ChishiTest] 主世界未就绪，跳过测试");
            server.halt(false);
            return;
        }
        ...
        ChishiFuelCannerBlockEntity canner = asCanner();
        ChishiFuelMixerBlockEntity mixer = asMixer();
        if (canner == null || mixer == null) {
            LOGGER.error("[ChishiTest] 机器方块实体未生成，跳过测试");
            server.halt(false);
            return;
        }
        ...
        canner.inventory().setItem(ChishiFuelCannerBlockEntity.INPUT_SLOT,
        ...
        ((ChishiEnergyStorage) mixer.getEnergyStorage()).setEnergy(ChishiFuelMixerBlockEntity.CHISHI_COST);

        phase = 0;
        ticks = 0;
        passed = 0;
        total = 0;
        LOGGER.info("[ChishiTest] 测试开始：装罐机 5000mb + 混合器终极配方，等待 {} tick 结算", TICKS_PER_PHASE);
    }
    ...
    private static void runPhase0() {
        ...
        ChishiFuelMixerBlockEntity mixer = asMixer();
        if (canner == null || mixer == null) {
            LOGGER.error("[ChishiTest] 阶段1：方块实体丢失，测试中止");
            finish();
            return;
        }
        Fluid ultimate = ModFluids.get(ModFluids.ULTIMATE_MIXTURE_FUEL_ID);
        ...
        LOGGER.info("[ChishiTest] ===== 阶段1 断言（半满罐 + 终极混合） =====");
        ItemStack input = canner.inventory().getItem(ChishiFuelCannerBlockEntity.INPUT_SLOT);
        ItemStack output = canner.inventory().getItem(ChishiFuelCannerBlockEntity.OUTPUT_SLOT);
        check(!input.isEmpty(), "[装罐机] 输入槽应保留半满燃料罐");
        check(ChishiFuelCellItem.getAmount(input) == 5000,
                "[装罐机] 半满罐应为 5000/10000，实际 " + ChishiFuelCellItem.getAmount(input));
        check(ChishiFuelCellItem.getFluid(input) == ultimate, "[装罐机] 罐内应为终极混合燃料");
        ...
        phase = 1;
        ticks = 0;
        LOGGER.info("[ChishiTest] 场景1b：补液 5000mb，等待 {} tick 结算", TICKS_PER_PHASE);
    }
    ...
    private static void runPhase1() {
        ChishiFuelCannerBlockEntity canner = asCanner();
        if (canner == null) {
            LOGGER.error("[ChishiTest] 阶段2：装罐机方块实体丢失，测试中止");
            finish();
            return;
        }
        LOGGER.info("[ChishiTest] ===== 阶段2 断言（补液灌满送出） =====");
        ItemStack out = canner.inventory().getItem(ChishiFuelCannerBlockEntity.OUTPUT_SLOT);
        check(!out.isEmpty(), "[装罐机] 输出槽应有满罐");
        check(ChishiFuelCellItem.getAmount(out) == ChishiFuelCellItem.CAPACITY,
                "[装罐机] 满罐应为 10000，实际 " + ChishiFuelCellItem.getAmount(out));
        check(canner.inventory().getItem(ChishiFuelCannerBlockEntity.INPUT_SLOT).isEmpty(),
                "[装罐机] 灌满后输入槽应被消耗");
        finish();
    }
    ...
    private static void check(boolean condition, String message) {
        ...
        if (condition) {
            passed++;
            LOGGER.info("[ChishiTest]   [PASS] {}", message);
        } else {
            LOGGER.error("[ChishiTest]   [FAIL] {}", message);
        }
    }
    ...
    private static void finish() {
        ...
        finished = true;
        LOGGER.info("[ChishiTest] 汇总: 通过 {} / 总 {} {}", passed, total,
                passed == total ? "（全部通过）" : "（存在失败）");
        server.halt(false);
    }

    private static ChishiFuelCannerBlockEntity asCanner() {
        return level.getBlockEntity(cannerPos) instanceof ChishiFuelCannerBlockEntity be ? be : null;
    }

    private static ChishiFuelMixerBlockEntity asMixer() {
        return level.getBlockEntity(mixerPos) instanceof ChishiFuelMixerBlockEntity be ? be : null;
    }
}
