 ...
 * 配方数据直接复用 {@link ChishiFuelMixerBlockEntity#recipeFor}，保证与机器逻辑一致。
...
public class FuelMixingRecipeCategory implements IRecipeCategory<FuelMixingRecipeCategory.FuelMixingRecipe> {
    ...
    public static final RecipeType<FuelMixingRecipe> TYPE =
            RecipeType.create(TemplateMod.MOD_ID, "fuel_mixing", FuelMixingRecipe.class);
    ...
    public Component getTitle() {
        return Component.translatable("jei.template_mod.fuel_mixing");
    }
    ...
    public void draw(FuelMixingRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
        ...
        guiGraphics.drawString(Minecraft.getInstance().font,
                Component.translatable("jei.template_mod.cost_mix"), 10, 51, 0xFFFFFFFF);
    }
    ...
    public record FuelMixingRecipe(Fluid in1, long in1Amount, Fluid in2, long in2Amount, Fluid out, long outAmount) {
        ...
        public static List<FuelMixingRecipe> getAll() {
            ...
            return List.of(
                    from(ChishiFuelMixerBlockEntity.recipeFor(end, compound)),
                    from(ChishiFuelMixerBlockEntity.recipeFor(dragon, pure)));
        }

        private static FuelMixingRecipe from(ChishiFuelMixerBlockEntity.Recipe r) {
