...
public class ForgingRecipeCategory implements IRecipeCategory<ForgingRecipeCategory.ForgingRecipe> {
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, ForgingRecipe recipe, IFocusGroup focuses) {
        // 输入：下界合金装备 + 赤石锭（数量按部位）；输出：对应赤石装备
        builder.addSlot(RecipeIngredientRole.INPUT, 8, 23).addIngredient(VanillaTypes.ITEM_STACK, recipe.base());
        builder.addSlot(RecipeIngredientRole.INPUT, 37, 23).addIngredient(VanillaTypes.ITEM_STACK, recipe.ingot());
    ...
    public void draw(ForgingRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
        guiGraphics.drawString(Minecraft.getInstance().font, Component.translatable("jei.template_mod.cost_forge"), 36, 48, 0xFF404040);
    }
