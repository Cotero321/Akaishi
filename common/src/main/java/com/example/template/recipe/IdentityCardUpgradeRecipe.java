...
public class IdentityCardUpgradeRecipe extends ShapelessRecipe {
    ...
    public IdentityCardUpgradeRecipe(ResourceLocation id) {
        super(id, "", CraftingBookCategory.MISC,
                new ItemStack(ModItems.chishiWirelessIdentityCard.get()),
                NonNullList.of(Ingredient.EMPTY,
                        Ingredient.of(ModItems.chishiWirelessIdentityCard.get()),
    ...
    public boolean matches(CraftingContainer inv, Level level) {
        ...
        for (int i = 0; i < inv.getContainerSize(); i++) {
            ...
            if (s.is(ModItems.chishiWirelessIdentityCard.get())) {
                if (!card.isEmpty()) {
                    return false; // 多张卡
                }
                card = s;
        ...
        IdentityCardTier tier = ChishiWirelessIdentityCardItem.tierOf(card);
        ChishiWirelessIdentityCardItem.UpgradeRequirement next = ChishiWirelessIdentityCardItem.nextUpgrade(tier);
    ...
    public ItemStack assemble(CraftingContainer inv, RegistryAccess registryAccess) {
        ...
        IdentityCardTier tier = ChishiWirelessIdentityCardItem.tierOf(card);
        IdentityCardTier next = ChishiWirelessIdentityCardItem.nextUpgrade(tier).nextTier();
        ItemStack out = card.copy(); // 复制全部 NBT（含卡号 UUID）
        ChishiWirelessIdentityCardItem.setTier(out, next);
        return out;
    }
