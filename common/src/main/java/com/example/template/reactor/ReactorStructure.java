package com.example.template.reactor;

import com.example.template.block.ChishiReactorControllerBlock;
import com.example.template.block.ChishiReactorCoolerBlock;
import com.example.template.block.ChishiReactorCoreBlock;
import com.example.template.block.ChishiReactorEnergyOutputBlock;
import com.example.template.block.ChishiReactorFuelPortBlock;
import com.example.template.block.ChishiReactorFuelRodBlock;
import com.example.template.block.ChishiReactorShellBlock;
import com.example.template.block.ChishiReactorWastePortBlock;
...
public final class ReactorStructure {
            ...
            for (int y = minY; y <= maxY; y++) {
                for (int z = minZ; z <= maxZ; z++) {
                    ...
                    if (onWall) {
                        ...
                        if (b instanceof ChishiReactorEnergyOutputBlock) {
                            energy.add(p);
                        } else if (b instanceof ChishiReactorWastePortBlock) {
                            waste.add(p);
                        } else if (b instanceof ChishiReactorFuelPortBlock) {
                            fuel.add(p);
                        } else if (b instanceof ChishiReactorShellBlock) {
                            shells.add(p);
                        }
                    } else if (level.getBlockState(p).isAir()) {
                        continue;
                    } else if (b instanceof ChishiReactorFuelRodBlock) {
                        rods++;
                    } else if (b instanceof ChishiReactorCoolerBlock) {
                        coolers++;
                        coolerList.add(p);
                        if (ChishiReactorCoolerBlockEntity.hasAdjacentFuelRod(level, p)) {
                            int q = ChishiReactorCoolerBlockEntity.getQualityAt(level, p);
                            if (q > 0) {
                                cooling += q;
                                coolersEffective++;
                                coolerEff.add(p);
                    ...
                    } else if (b instanceof ChishiReactorCoreBlock) {
                        cores++;
                    } else {
    ...
    public static boolean isWallBlock(Block b) {
        return b instanceof ChishiReactorShellBlock
                || b instanceof ChishiReactorControllerBlock
                || b instanceof ChishiReactorFuelPortBlock
                || b instanceof ChishiReactorEnergyOutputBlock
                || b instanceof ChishiReactorWastePortBlock;
    }
}
