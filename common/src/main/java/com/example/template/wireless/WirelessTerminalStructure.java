package com.example.template.wireless;

import com.example.template.block.ChishiWirelessChunkLoaderBlock;
import com.example.template.block.ChishiWirelessChunkRangeBlock;
import com.example.template.block.ChishiWirelessControllerBlock;
import com.example.template.block.ChishiWirelessCoreBlock;
import com.example.template.block.ChishiWirelessDimBridgeBlock;
import com.example.template.block.ChishiWirelessInputLossBlock;
import com.example.template.block.ChishiWirelessOutputLossBlock;
import com.example.template.block.ChishiWirelessSecurityBlock;
import com.example.template.block.ChishiWirelessShellBlock;
...
public final class WirelessTerminalStructure {
            ...
            for (int y = minY; y <= maxY; y++) {
                for (int z = minZ; z <= maxZ; z++) {
                    ...
                    if (onWall) {
                        ...
                        if (b instanceof ChishiWirelessShellBlock) {
                            shells.add(p);
                        }
                    } else if (level.getBlockState(p).isAir()) {
                        continue;
                    } else if (b instanceof ChishiWirelessCoreBlock) {
                        if (core != null) {
                            return null; // 内腔多于一个终端核心
                        }
                        core = p;
                    } else if (b instanceof ChishiWirelessDimBridgeBlock) {
                        crossDimCount++;
                    } else if (b instanceof ChishiWirelessChunkLoaderBlock) {
                        chunkLoaderCount++;
                    } else if (b instanceof ChishiWirelessChunkRangeBlock) {
                        chunkRangeCount++;
                    } else if (b instanceof ChishiWirelessInputLossBlock) {
                        inputLossCount++;
                    } else if (b instanceof ChishiWirelessOutputLossBlock) {
                        outputLossCount++;
                    } else {
    ...
    public static boolean isWallBlock(Block b) {
        return b instanceof ChishiWirelessTerminalBlock
                || b instanceof ChishiWirelessSecurityBlock
                || b instanceof ChishiWirelessControllerBlock
                || b instanceof ChishiWirelessShellBlock;
    }
}
