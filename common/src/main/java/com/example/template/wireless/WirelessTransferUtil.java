...
public final class WirelessTransferUtil {
     ...
     */
    public static double lossRatio(Level level, BlockPos portPos, ChishiWirelessTerminalBlockEntity terminal, boolean isOutput) {
     ...
     */
    public static ChishiWirelessTerminalBlockEntity resolveTerminal(Level level, UUID boundCard) {
        ...
        if (tr.dimension().equals(level.dimension())) {
            return level.getBlockEntity(tr.pos()) instanceof ChishiWirelessTerminalBlockEntity t && t.isFormed()
                    ? t : null;
        }
        // 跨维度：需终端解锁跨维组件
        ...
        return target.getBlockEntity(tr.pos()) instanceof ChishiWirelessTerminalBlockEntity t && t.isFormed()
                ? t : null;
    }
    ...
    private static boolean crossDimUnlocked(Level level, WirelessNetworkManager.TerminalRef tr) {
        ...
        return target.getBlockEntity(tr.pos()) instanceof ChishiWirelessTerminalBlockEntity t && t.isCrossDim();
    }
