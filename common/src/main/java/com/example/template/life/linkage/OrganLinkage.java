package com.example.template.life.linkage;
...
public final class OrganLinkage {
    ...
    public static double rejectionFactorOf(ItemStack organ) {
        return rejectionFactorOf(ChishiOrganItem.getSource(organ));
    }

    /**
     * 器官移植排斥总计算 = 品质基础排斥 × 分组排斥系数 × (1 − 完整度修正)。
     * 完整度越高排斥越低（基因质量 → 契合质量），原生器官恒为 0。
     * ChishiOrganItem.getBaseRejection 委托本方法，移植/冲突解除/tooltip 自动同步。
     */
    public static int rejectionOf(ItemStack organ) {
        ...
        double purityMod = 1.0 - (ChishiOrganItem.getPurity(organ) / 100.0) * PURITY_REJECTION_CAP;
    ...
    public static int compatRoll(SampleGroup source, int purity, Random random) {
        if (source == null) {
            return ChishiOrganItem.NATIVE_COMPAT;
        }
        ...
        return Math.min(ChishiOrganItem.MAX_COMPAT, source.getCompatMin() + (int) (range * roll));
    }
