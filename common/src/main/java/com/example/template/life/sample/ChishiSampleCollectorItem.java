 ...
 */
public class ChishiSampleCollectorItem extends Item {
    ...
    public ChishiSampleCollectorItem(Properties properties) {
        super(properties);
    }
        ...
        if (level.getBlockEntity(ctx.getClickedPos()) instanceof IEnergyProvider provider) {
            ...
            if (storage != null) {
                ...
                if (toExtract > 0) {
                    ...
                    player.sendSystemMessage(Component.translatable("message.template_mod.sample.charged",
    ...
    public static boolean tryCollect(Player player, LivingEntity target, ItemStack collector) {
        if (collector.isEmpty() || !(collector.getItem() instanceof ChishiSampleCollectorItem)) {
            return false;
        }
        ...
        if (stored < COST_PER_SAMPLE) {
            player.sendSystemMessage(Component.translatable("message.template_mod.sample.no_energy"));
            return false;
        }
        ...
        // 成功：全额扣能量 + 生成样本（纯度偏斜规则见 ChishiLifeSampleItem.createRolled）
        if (player.getRandom().nextInt(100) < rate) {
            ...
            ItemStack sample = ChishiLifeSampleItem.createRolled(group, entityId, player.getRandom());
            if (!player.getInventory().add(sample)) {
                player.drop(sample, false);
            }
            player.sendSystemMessage(Component.translatable("message.template_mod.sample.collect",
                    Component.translatable(group.getNameKey()), ChishiLifeSampleItem.getPurity(sample)));
        } else {
            // 失败：仅消耗一半能量，样本流失
            setEnergy(collector, stored - COST_PER_SAMPLE / 2);
            player.sendSystemMessage(Component.translatable("message.template_mod.sample.fail"));
        }
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tooltip, TooltipFlag flag) {
        ...
        tooltip.add(Component.translatable("gui.template_mod.sample_collector.hint"));
        tooltip.add(Component.translatable("gui.template_mod.sample_collector.charge_hint"));
    }
}
