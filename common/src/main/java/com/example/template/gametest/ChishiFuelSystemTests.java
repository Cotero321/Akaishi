package com.example.template.gametest;
...
/**
 * 燃料系统端到端测试（GameTest，dev 环境以 -Dchishi.gametest=1 触发）。
 * 单结构内放置装罐机 + 混合器，顺序验证：
 * 1. 装罐机：5000mb 终极混合燃料 → 半满罐（5000/10000 留在输入槽）；补液灌满 → 满罐送出
 * 2. 混合器：巨龙 100mb + 至纯 100mb + 2M 赤能源 → 终极混合燃料 50mb（1:1:1 消耗）
 */
public final class ChishiFuelSystemTests {

    private ChishiFuelSystemTests() {
    }
    ...
    public static void fuelSystemEndToEnd(GameTestHelper helper) {
        ...
        helper.runAfterDelay(1, () -> {
            ChishiFuelCannerBlockEntity canner = asCanner(helper, cannerPos);
            ChishiFuelMixerBlockEntity mixer = asMixer(helper, mixerPos);
            ...
            helper.runAfterDelay(20, () -> {
                ...
                helper.assertTrue(ChishiFuelCellItem.getAmount(input) == 5000,
                        "[装罐机] 半满罐应为 5000/10000，实际 " + ChishiFuelCellItem.getAmount(input));
                helper.assertTrue(ChishiFuelCellItem.getFluid(input) == ultimate, "[装罐机] 罐内应为终极混合燃料");
                ...
                helper.runAfterDelay(20, () -> {
                    ...
                    helper.assertTrue(ChishiFuelCellItem.getAmount(out) == ChishiFuelCellItem.CAPACITY,
                            "[装罐机] 满罐应为 10000，实际 " + ChishiFuelCellItem.getAmount(out));
                    helper.assertTrue(canner.inventory().getItem(ChishiFuelCannerBlockEntity.INPUT_SLOT).isEmpty(),
                            "[装罐机] 灌满后输入槽应被消耗");
                    helper.succeed();
                });
            });
    ...
    private static ChishiFuelCannerBlockEntity asCanner(GameTestHelper helper, BlockPos pos) {
        return helper.getBlockEntity(pos) instanceof ChishiFuelCannerBlockEntity be ? be : null;
    }

    private static ChishiFuelMixerBlockEntity asMixer(GameTestHelper helper, BlockPos pos) {
        return helper.getBlockEntity(pos) instanceof ChishiFuelMixerBlockEntity be ? be : null;
    }
}
