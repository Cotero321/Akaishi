 ...
 */
public class ChishiGatheringBracelet extends ChishiCurioItem {
    ...
    public ChishiGatheringBracelet(Properties properties) {
    ...
    protected String tooltipKey() {
        return "item.template_mod.curio.gathering";
    }
    ...
    public void onBlockBreak(Player player, ItemStack stack, BlockPos pos, BlockState state) {
        ...
        if (!level.isClientSide && level.getRandom().nextInt(100) < DROP_CHANCE_PERCENT) {
            Block.popResource(level, pos, new ItemStack(ModItems.chishiCrystal.get()));
        }
    }
}
