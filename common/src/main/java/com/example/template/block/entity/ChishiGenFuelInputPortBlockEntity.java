 ...
 */
public class ChishiGenFuelInputPortBlockEntity extends BlockEntity implements IItemPipeDevice, IDataCarrier {
    ...
    public ChishiGenFuelInputPortBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.CHISHI_GEN_FUEL_INPUT.get(), pos, state);
        this.buffer = new SimpleContainer(BUFFER_SLOTS) {
            @Override
            public void setChanged() {
                super.setChanged();
                ChishiGenFuelInputPortBlockEntity.this.setChanged();
            }
        };
    }

    public static void serverTick(Level level, BlockPos pos, BlockState state, ChishiGenFuelInputPortBlockEntity be) {
        be.tickServer();
    }
    ...
    private void pushFuelToController(ChishiGenMatrixControllerBlockEntity controller) {
        if (!controller.getItem(ChishiGenMatrixControllerBlockEntity.FUEL_SLOT).isEmpty()) {
            return;
        }
        for (int i = 0; i < BUFFER_SLOTS; i++) {
            ItemStack stack = buffer.getItem(i);
            if (!stack.isEmpty() && ChishiFuels.getFuelEnergy(stack) > 0) {
                controller.setItem(ChishiGenMatrixControllerBlockEntity.FUEL_SLOT, stack.copyWithCount(1));
                stack.shrink(1);
                if (stack.isEmpty()) {
    ...
    public boolean canPlaceItem(int index, ItemStack stack) {
        return stack.isEmpty() || ChishiFuels.getFuelEnergy(stack) > 0;
    }
