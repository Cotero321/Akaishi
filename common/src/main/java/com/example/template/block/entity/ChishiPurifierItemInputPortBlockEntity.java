 ...
 */
public class ChishiPurifierItemInputPortBlockEntity extends BlockEntity implements IItemPipeDevice, IDataCarrier {
    ...
    public ChishiPurifierItemInputPortBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.CHISHI_PURIFIER_ITEM_INPUT.get(), pos, state);
        this.buffer = new SimpleContainer(BUFFER_SLOTS) {
            @Override
            public void setChanged() {
                super.setChanged();
                ChishiPurifierItemInputPortBlockEntity.this.setChanged();
            }
        };
    }

    public static void serverTick(Level level, BlockPos pos, BlockState state, ChishiPurifierItemInputPortBlockEntity be) {
        be.tickServer();
    }
    ...
    private void pushInputToController(ChishiPurifierMatrixControllerBlockEntity controller) {
        if (!controller.getItem(ChishiPurifierMatrixControllerBlockEntity.INPUT_SLOT).isEmpty()) {
            return;
        }
        for (int i = 0; i < BUFFER_SLOTS; i++) {
            ItemStack stack = buffer.getItem(i);
            if (!stack.isEmpty() && isValidInput(stack)) {
                controller.setItem(ChishiPurifierMatrixControllerBlockEntity.INPUT_SLOT, stack.copyWithCount(1));
                stack.shrink(1);
                if (stack.isEmpty()) {
