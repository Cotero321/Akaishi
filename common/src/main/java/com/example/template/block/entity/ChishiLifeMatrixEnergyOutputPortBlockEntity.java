 ...
 */
public class ChishiLifeMatrixEnergyOutputPortBlockEntity extends BlockEntity implements IEnergyProvider, IDataCarrier {
    ...
    public ChishiLifeMatrixEnergyOutputPortBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.CHISHI_LIFE_MATRIX_ENERGY_OUTPUT.get(), pos, state);
        this.energy = new ChishiEnergyStorage(LifeEnergyType.INSTANCE, BUFFER_CAPACITY);
    }

    public static void serverTick(Level level, BlockPos pos, BlockState state, ChishiLifeMatrixEnergyOutputPortBlockEntity be) {
        be.tickServer();
    }
    ...
    private void tickServer() {
        ...
        long rate = ChishiLifeMatrixControllerBlockEntity.CONVERSIONS_PER_TICK
                * ChishiLifeMatrixControllerBlockEntity.CONVERSION_OUTPUT;
