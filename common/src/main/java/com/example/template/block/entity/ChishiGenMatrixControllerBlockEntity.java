...
import com.example.template.block.ChishiGenEnergyOutputPortBlock;
import com.example.template.block.ChishiGenFuelInputPortBlock;
import com.example.template.block.ChishiGenMatrixCasingBlock;
...
import com.example.template.menu.ChishiGenMatrixControllerMenu;
 ...
 */
public class ChishiGenMatrixControllerBlockEntity extends BlockEntity implements ExtendedMenuProvider, IEnergyProvider, Container, IDataCarrier {
    ...
    public ChishiGenMatrixControllerBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.CHISHI_GEN_MATRIX_CONTROLLER.get(), pos, state);
        this.energy = new ChishiEnergyStorage(ChishiEnergyType.INSTANCE, tier().maxEnergy);
        this.inventory = new SimpleContainer(TOTAL_SLOTS) {
            @Override
            public void setChanged() {
                super.setChanged();
                ChishiGenMatrixControllerBlockEntity.this.setChanged();
            }
        };
    }

    /** 矩阵等级（由方块实例决定，低级/高级） */
    public ChishiGenMatrixTier tier() {
        Block b = getBlockState().getBlock();
        return b instanceof ChishiGenMatrixControllerBlock c ? c.tier() : ChishiGenMatrixTier.BASIC;
    }
    ...
    public static void serverTick(Level level, BlockPos pos, BlockState state, ChishiGenMatrixControllerBlockEntity be) {
        be.tickServer();
    }
    ...
    private void tickServer() {
        ...
        boolean formed = getBlockState().getValue(ChishiGenMatrixControllerBlock.FORMED);
    ...
    private boolean isWall(Block b) {
        return b instanceof ChishiGenMatrixCasingBlock
                || b instanceof ChishiGenEnergyOutputPortBlock
                || b instanceof ChishiGenFuelInputPortBlock
                || b instanceof ChishiGenMatrixControllerBlock;
    }
    ...
    private void setFormed(boolean formed, MatrixStructure.Result scan) {
        level.setBlock(worldPosition, getBlockState().setValue(ChishiGenMatrixControllerBlock.FORMED, formed), 3);
        // 解除关联时遍历上次成型的箱体范围
        ...
        for (BlockPos p : BlockPos.betweenClosed(min, max)) {
            ...
            if (be instanceof ChishiGenEnergyOutputPortBlockEntity o) {
                o.setControllerPos(link);
            } else if (be instanceof ChishiGenFuelInputPortBlockEntity f) {
    ...
    public boolean isFormed() {
        return getBlockState().getValue(ChishiGenMatrixControllerBlock.FORMED);
    }
    ...
    public Component getDisplayName() {
        return Component.translatable("block.template_mod.chishi_gen_matrix_controller_" + tier().suffix);
    }

    @Override
    public AbstractContainerMenu createMenu(int id, Inventory inv, Player player) {
        return new ChishiGenMatrixControllerMenu(id, inv, this);
    }
