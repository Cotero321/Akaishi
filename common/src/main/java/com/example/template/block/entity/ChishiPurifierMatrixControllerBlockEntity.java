...
import com.example.template.block.ChishiPurifierEnergyInputPortBlock;
import com.example.template.block.ChishiPurifierItemInputPortBlock;
import com.example.template.block.ChishiPurifierItemOutputPortBlock;
import com.example.template.block.ChishiPurifierMatrixCasingBlock;
...
import com.example.template.menu.ChishiPurifierMatrixControllerMenu;
 ...
 */
public class ChishiPurifierMatrixControllerBlockEntity extends BlockEntity
    ...
    public ChishiPurifierMatrixControllerBlockEntity(BlockPos pos, BlockState state) {
        ...
        this.inventory = new SimpleContainer(SLOT_COUNT) {
            ...
            public void setChanged() {
                super.setChanged();
                ChishiPurifierMatrixControllerBlockEntity.this.setChanged();
            }
        };
    }

    public static void serverTick(Level level, BlockPos pos, BlockState state, ChishiPurifierMatrixControllerBlockEntity be) {
        be.tickServer();
    }
    ...
    private void tickServer() {
        ...
        boolean formed = getBlockState().getValue(ChishiPurifierMatrixControllerBlock.FORMED);
        ...
        // 成型后集中提纯：消耗能量推进进度，不足时暂停不清零
        if (formed && canProcess()) {
            long extract = Math.min(RATE_FORMED, energy.getEnergyStored());
            if (extract > 0) {
                ...
                if (progressEnergy >= TOTAL_COST) {
                    ...
                    if (out.isEmpty()) {
                        inventory.setItem(OUTPUT_SLOT, new ItemStack(ModItems.chishiEssence.get(), amount));
                    } else {
                        out.grow(amount);
                    }
                }
    ...
    private boolean isWall(Block b) {
        return b instanceof ChishiPurifierMatrixCasingBlock
                || b instanceof ChishiPurifierItemInputPortBlock
                || b instanceof ChishiPurifierItemOutputPortBlock
                || b instanceof ChishiPurifierEnergyInputPortBlock
                || b instanceof ChishiPurifierMatrixControllerBlock;
    }
    ...
    private void setFormed(boolean formed, MatrixStructure.Result scan) {
        level.setBlock(worldPosition, getBlockState().setValue(ChishiPurifierMatrixControllerBlock.FORMED, formed), 3);
        // 解除关联时遍历上次成型的箱体范围
        ...
        for (BlockPos p : BlockPos.betweenClosed(min, max)) {
            ...
            if (be instanceof ChishiPurifierItemInputPortBlockEntity in) {
                in.setControllerPos(link);
            } else if (be instanceof ChishiPurifierItemOutputPortBlockEntity out) {
                out.setControllerPos(link);
            } else if (be instanceof ChishiPurifierEnergyInputPortBlockEntity e) {
    ...
    public boolean isFormed() {
        return getBlockState().getValue(ChishiPurifierMatrixControllerBlock.FORMED);
    }
    ...
    public Component getDisplayName() {
        return Component.translatable("block.template_mod.chishi_purifier_matrix_controller");
    }

    @Override
    public AbstractContainerMenu createMenu(int id, Inventory inv, Player player) {
        return new ChishiPurifierMatrixControllerMenu(id, inv, this);
    }
