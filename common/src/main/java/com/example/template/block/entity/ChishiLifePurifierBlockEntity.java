...
import com.example.template.menu.ChishiLifePurifierMenu;
 ...
 * 槽位：0=输出（生命能量固态物）。
 */
public class ChishiLifePurifierBlockEntity extends BlockEntity implements ExtendedMenuProvider, IEnergyProvider, IItemPipeDevice {
    ...
    public ChishiLifePurifierBlockEntity(BlockPos pos, BlockState state) {
        ...
        this.inventory = new SimpleContainer(SLOT_COUNT) {
            ...
            public void setChanged() {
                super.setChanged();
                ChishiLifePurifierBlockEntity.this.setChanged();
            }
        };
        this.data = new SimpleContainerData(DATA_SLOTS);
    }

    public static void serverTick(Level level, BlockPos pos, BlockState state, ChishiLifePurifierBlockEntity be) {
        be.tickServer();
    }
        ...
        if (canProcess()) {
            ...
            if (extract > 0) {
                ...
                if (progressEnergy >= TOTAL_COST) {
                    ...
                    if (out.isEmpty()) {
                        inventory.setItem(OUTPUT_SLOT, new ItemStack(ModItems.chishiLifeEssenceSolid.get()));
                    } else {
                        out.grow(1);
                    }
                }
    ...
    private boolean canProcess() {
        ...
        return out.is(ModItems.chishiLifeEssenceSolid.get()) && out.getCount() < out.getMaxStackSize();
    }
    ...
    public boolean canInputEnergy(IEnergyType type) {
        return type == ChishiEnergyType.INSTANCE || type == LifeEnergyType.INSTANCE;
    }
    ...
    public Component getDisplayName() {
        return Component.translatable("block.template_mod.chishi_life_purifier");
    }

    @Override
    public AbstractContainerMenu createMenu(int id, Inventory inv, Player player) {
        return new ChishiLifePurifierMenu(id, inv, this);
    }
