...
import com.example.template.block.ChishiEnergyCellSerializerBlock;
...
import com.example.template.menu.ChishiEnergyCellSerializerMenu;
...
public class ChishiEnergyCellSerializerBlockEntity extends BlockEntity implements ExtendedMenuProvider, IEnergyProvider, IDataCarrier {
    ...
    private final ChishiEnergyCellArrayStorage arrayStorage;
    private final SimpleContainerData data;

    public ChishiEnergyCellSerializerBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.CHISHI_ENERGY_CELL_SERIALIZER.get(), pos, state);
        this.energy = new ChishiEnergyStorage(ChishiEnergyType.INSTANCE, BASE_CAPACITY);
        this.arrayStorage = new ChishiEnergyCellArrayStorage(ChishiEnergyType.INSTANCE, this::collectMembers);
        this.data = new SimpleContainerData(5);
    }

    public static void serverTick(Level level, BlockPos pos, BlockState state, ChishiEnergyCellSerializerBlockEntity be) {
        be.tickServer();
    }
    ...
    private void tickServer() {
        ...
        if (formed != blockState.getValue(ChishiEnergyCellSerializerBlock.FORMED)) {
            level.setBlock(worldPosition, blockState.setValue(ChishiEnergyCellSerializerBlock.FORMED, formed), 3);
        }
        ...
        for (int dx = -1; dx <= 1; dx++) {
            for (int dy = -1; dy <= 1; dy++) {
                for (int dz = -1; dz <= 1; dz++) {
                    ...
                    if (!(level.getBlockState(worldPosition.offset(dx, dy, dz)).getBlock() instanceof ChishiEnergyCellBlock)) {
                        return false;
                    }
                }
            }
    ...
    public Component getDisplayName() {
        return Component.translatable("block.template_mod.chishi_energy_cell_serializer");
    }

    @Override
    public AbstractContainerMenu createMenu(int id, Inventory inv, Player player) {
        return new ChishiEnergyCellSerializerMenu(id, inv, data);
    }
