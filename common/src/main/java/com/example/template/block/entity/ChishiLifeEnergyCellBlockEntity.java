...
public class ChishiLifeEnergyCellBlockEntity extends BlockEntity implements ExtendedMenuProvider, IEnergyProvider {
    ...
    public ChishiLifeEnergyCellBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.CHISHI_LIFE_ENERGY_CELL.get(), pos, state);
        this.energy = new ChishiEnergyStorage(LifeEnergyType.INSTANCE, LIFE_CAPACITY);
        this.data = new SimpleContainerData(5);
    }

    public static void serverTick(Level level, BlockPos pos, BlockState state, ChishiLifeEnergyCellBlockEntity be) {
        be.tickServer();
    }
    ...
    public Component getDisplayName() {
        return Component.translatable("block.template_mod.chishi_life_energy_cell");
    }
