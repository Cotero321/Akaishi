 ...
 */
public class ChishiPurifierItemOutputPortBlockEntity extends BlockEntity implements IItemPipeDevice, IDataCarrier {
    ...
    public ChishiPurifierItemOutputPortBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.CHISHI_PURIFIER_ITEM_OUTPUT.get(), pos, state);
        this.buffer = new SimpleContainer(BUFFER_SLOTS) {
            @Override
            public void setChanged() {
                super.setChanged();
                ChishiPurifierItemOutputPortBlockEntity.this.setChanged();
            }
        };
    }

    public static void serverTick(Level level, BlockPos pos, BlockState state, ChishiPurifierItemOutputPortBlockEntity be) {
        be.tickServer();
    }
    ...
    private void pullOutputFromController(ChishiPurifierMatrixControllerBlockEntity controller) {
        ItemStack out = controller.getItem(ChishiPurifierMatrixControllerBlockEntity.OUTPUT_SLOT);
        if (out.isEmpty() || !out.is(ModItems.chishiEssence.get())) {
            return;
        }
