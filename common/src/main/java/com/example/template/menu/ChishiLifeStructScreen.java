 ...
 * - 顶部显示当前基因序列的来源生物
 * 数据来自 {@link ChishiLifeStructMenu} 的 ContainerData。
 */
public class ChishiLifeStructScreen extends AbstractContainerScreen<ChishiLifeStructMenu> {

    private static final ResourceLocation TEXTURE = new ResourceLocation(TemplateMod.MOD_ID, "textures/gui/chishi_life_struct.png");
    ...
    public ChishiLifeStructScreen(ChishiLifeStructMenu menu, Inventory inv, Component title) {
        ...
        if (button == 0 && menu.getBlockPos() != null) {
            ...
            for (int i = 0; i < BodySlot.values().length; i++) {
                ...
                if (mouseX >= bx - 1 && mouseX < bx + BTN_SIZE + 1
                    ...
                    ChishiLifeStructSync.sendSelect(menu.getBlockPos(), i);
                    return true;
                }
            }
        }
    ...
    public void render(GuiGraphics gui, int mouseX, int mouseY, float partialTick) {
        ...
        if (isHovering(PROGRESS_X, PROGRESS_Y, PROGRESS_W, PROGRESS_H, mouseX, mouseY)) {
            ...
            tip.add(Component.translatable("gui.template_mod.life_struct.progress", menu.getProgress()));
            String entityId = menu.getSequenceEntityId();
            if (entityId != null) {
                tip.add(Component.translatable("gui.template_mod.life_struct.gene",
                        Component.translatable("entity." + entityId.replace(':', '.'))));
            }
            tip.add(Component.translatable("gui.template_mod.life_struct.cost"));
        ...
        for (int i = 0; i < slots.length; i++) {
            ...
            if (mouseX >= bx - 1 && mouseX < bx + BTN_SIZE + 1
                ...
                if (i == menu.getTargetSlot()) {
                    tip = Component.translatable("gui.template_mod.life_struct.selected", tip);
                } else if (!menu.getAvailableSlots().contains(slots[i])) {
                    tip = Component.translatable("gui.template_mod.life_struct.unavailable", tip);
                }
