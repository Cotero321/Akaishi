...
/**
 * 生命分析台界面：生命能量条（绿）+ 解构进度条（黄）+ 输入/输出槽。
 * 数据来自 {@link ChishiGeneAnalyzerMenu} 的 ContainerData。
 */
public class ChishiGeneAnalyzerScreen extends AbstractContainerScreen<ChishiGeneAnalyzerMenu> {
    ...
    public ChishiGeneAnalyzerScreen(ChishiGeneAnalyzerMenu menu, Inventory inv, Component title) {
    ...
    public void render(GuiGraphics gui, int mouseX, int mouseY, float partialTick) {
        ...
        if (isHovering(PROGRESS_X, PROGRESS_Y, PROGRESS_W, PROGRESS_H, mouseX, mouseY)) {
            gui.renderTooltip(this.font,
                    Component.translatable("gui.template_mod.gene_analyzer.progress",
