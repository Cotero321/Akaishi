 ...
 * 数据来自 {@link ChishiWirelessPortMenu} 的 ContainerData（缓冲储能 long + 绑定卡/终端/认证/速率）。
 * 198 高布局：标题 y=6 与切页按钮 y=16 错开，内容区 y=36 起舒展排布。
 */
public class ChishiWirelessPortScreen extends AbstractContainerScreen<ChishiWirelessPortMenu> {
    ...
    public ChishiWirelessPortScreen(ChishiWirelessPortMenu menu, Inventory inv, Component title) {
    ...
    protected void renderLabels(GuiGraphics gui, int mouseX, int mouseY) {
        ...
        drawTabLabel(gui, 88, "gui.template_mod.wireless.tab.transfer", currentPage == 1);
    ...
    private void renderRunPage(GuiGraphics gui) {
        ...
        if (menu.isAuthenticated()) {
            gui.drawString(this.font, Component.translatable("gui.template_mod.wireless.port.authed",
                    String.format("%08X", menu.getTerminalHash())),
                    x + 8, y + 48, TEXT_GREEN, false);
            // 区块弱加载提示（认证后由终端区块加载构架统一管理）
            gui.drawString(this.font, Component.translatable("gui.template_mod.wireless.port.chunk"),
                    x + 8, y + 60, TEXT_DIM, false);
        } else {
            gui.drawString(this.font, Component.translatable(bound
                            ? "gui.template_mod.wireless.port.not_authed" : "gui.template_mod.wireless.port.no_card"),
                    x + 8, y + 48, bound ? TEXT_RED : TEXT_DIM, false);
            if (bound) {
                gui.drawString(this.font, Component.translatable("gui.template_mod.wireless.port.bind_hint"),
                        x + 8, y + 60, TEXT_DIM, false);
            }
        }
        // 解绑按钮
        if (bound) {
            ...
            gui.drawString(this.font, Component.translatable("gui.template_mod.wireless.port.unbind"),
    ...
    private void renderTransferPage(GuiGraphics gui) {
        ...
        // 传输速率（按卡自动解锁）
        gui.drawString(this.font, Component.translatable("gui.template_mod.wireless.port.rate",
        ...
        gui.drawString(this.font, Component.translatable(menu.isOutput()
                        ? "gui.template_mod.wireless.port.output_hint" : "gui.template_mod.wireless.port.input_hint"),
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (button == 0) {
            ...
            if (currentPage == 0 && menu.getCardHash() != 0
                ...
                this.minecraft.gameMode.handleInventoryButtonClick(this.menu.containerId, ChishiWirelessPortMenu.BTN_UNBIND);
                return true;
            }
        }
