 ...
 */
public class ChishiPotionCabinetScreen extends AbstractContainerScreen<ChishiPotionCabinetMenu> {
    ...
    public ChishiPotionCabinetScreen(ChishiPotionCabinetMenu menu, Inventory inv, Component title) {
    ...
    protected void renderBg(GuiGraphics gui, float partialTick, int mouseX, int mouseY) {
        ...
        int bx = 8;
        bx = drawFilterButton(gui, x + bx, y, "", "gui.template_mod.potion_cabinet.all", filter);
        ...
        if (button == 0 && mouseY >= this.topPos + BTN_Y && mouseY < this.topPos + BTN_Y + BTN_H) {
            ...
            for (String id : filters) {
                String nameKey = id.isEmpty()
                        ? "gui.template_mod.potion_cabinet.all"
