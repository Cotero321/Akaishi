...
/**
 * 小型赤能源组合结构界面：顶部状态条 + 燃料槽 + 火焰 + 能量条。
 * 结构激活时显示当前产能（45 倍），未激活时提示结构不完整。
 */
public class ChishiEnergyAssemblyScreen extends AbstractContainerScreen<ChishiEnergyAssemblyMenu> {
    ...
    public ChishiEnergyAssemblyScreen(ChishiEnergyAssemblyMenu menu, Inventory inv, Component title) {
    ...
    protected void renderBg(GuiGraphics gui, float partialTick, int mouseX, int mouseY) {
        ...
        int maxEnergy = ChishiEnergyAssemblyBlockEntity.MAX_ENERGY;
    ...
    public void render(GuiGraphics gui, int mouseX, int mouseY, float partialTick) {
        ...
        if (menu.isFormed()) {
            Component status = Component.translatable("gui.template_mod.assembly.formed", ChishiEnergyAssemblyBlockEntity.GENERATE_RATE);
            gui.drawString(this.font, status, sx + 3, sy + 3, 0xFF55E050, false);
        } else {
            Component status = Component.translatable("gui.template_mod.assembly.unformed");
            gui.drawString(this.font, status, sx + 3, sy + 3, 0xFFE0C040, false);
        }

        if (isHovering(154, 39, 10, 38, mouseX, mouseY)) {
            gui.renderTooltip(this.font,
                    Component.translatable("gui.template_mod.energy", menu.getEnergy(), ChishiEnergyAssemblyBlockEntity.MAX_ENERGY),
                    mouseX, mouseY);
            ...
            gui.renderTooltip(this.font,
                    menu.isFormed()
                            ? Component.translatable("gui.template_mod.assembly.formed_hint", ChishiEnergyAssemblyBlockEntity.GENERATE_RATE)
                            : Component.translatable("gui.template_mod.assembly.unformed_hint"),
                    mouseX, mouseY);
        }
    }
}
