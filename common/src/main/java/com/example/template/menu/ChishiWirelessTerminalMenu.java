 ...
 */
public class ChishiWirelessTerminalMenu extends AbstractContainerMenu {
    ...
    private final ChishiWirelessTerminalBlockEntity be;
    /** 初始页面（安全方块/终端方块经网络缓冲传入，Screen 打开时定位） */
    private int initialPage;

    /** 服务端构造：持有方块实体（授权/移除授权在此生效） */
    public ChishiWirelessTerminalMenu(int id, Inventory inv, ChishiWirelessTerminalBlockEntity be) {
    ...
    public ChishiWirelessTerminalMenu(int id, Inventory inv, ContainerData data) {
        super(ModMenus.CHISHI_WIRELESS_TERMINAL.get(), id);
        this.be = null;
        this.data = data;
        this.cardInv = new SimpleContainer(1);
    ...
    private void addCardSlot() {
        this.addSlot(new Slot(cardInv, 0, CARD_SLOT_X, CARD_SLOT_Y) {
            ...
            public boolean mayPlace(ItemStack stack) {
                return stack.getItem() instanceof ChishiWirelessIdentityCardItem;
            }
    ...
    private void addPlayerSlots(Inventory inv) {
        // 198 高 GUI：玩家背包 3 行下移至 y=124 起，快捷栏 y=180（与 chishi_wireless_terminal.png 槽位图案对齐）
    ...
    public boolean clickMenuButton(Player player, int id) {
        ...
        if (card.getItem() instanceof ChishiWirelessIdentityCardItem) {
            if (id == BTN_AUTHORIZE) {
                be.authorizeCard(ChishiWirelessIdentityCardItem.uuidOf(card));
                return true;
            }
            if (id == BTN_REVOKE) {
                be.revokeCard(ChishiWirelessIdentityCardItem.uuidOf(card));
                return true;
            }
        }
        return false;
    ...
    public long getEnergy() {
        return ((long) data.get(ChishiWirelessTerminalBlockEntity.DATA_STORED_HIGH) << 32)
                | (data.get(ChishiWirelessTerminalBlockEntity.DATA_STORED_LOW) & 0xFFFFFFFFL);
    }

    public long getMaxEnergy() {
        return ((long) data.get(ChishiWirelessTerminalBlockEntity.DATA_CAPACITY_HIGH) << 32)
                | (data.get(ChishiWirelessTerminalBlockEntity.DATA_CAPACITY_LOW) & 0xFFFFFFFFL);
    }

    public boolean isFormed() {
        return data.get(ChishiWirelessTerminalBlockEntity.DATA_FORMED) == 1;
    }

    public int getInputCount() {
        return data.get(ChishiWirelessTerminalBlockEntity.DATA_INPUT_COUNT);
    }

    public int getOutputCount() {
        return data.get(ChishiWirelessTerminalBlockEntity.DATA_OUTPUT_COUNT);
    }

    public int getBoundSerializers() {
        return data.get(ChishiWirelessTerminalBlockEntity.DATA_BOUND_SERIALIZERS);
    }

    public int getAuthorizedCount() {
        return data.get(ChishiWirelessTerminalBlockEntity.DATA_AUTHORIZED);
    }

    /** 跨维度是否已解锁（内腔含终端跨维组件） */
    public boolean isCrossDim() {
        return data.get(ChishiWirelessTerminalBlockEntity.DATA_CROSS_DIM) == 1;
    }

    /** 区块加载是否已启用（内腔含区块加载构架） */
    public boolean isChunkLoad() {
        return data.get(ChishiWirelessTerminalBlockEntity.DATA_CHUNK_LOAD) == 1;
    }

    /** 区块加载范围是否已扩展为 3×3（内腔含区块加载扩展组件） */
    public boolean isChunkRange() {
        return data.get(ChishiWirelessTerminalBlockEntity.DATA_CHUNK_RANGE) == 1;
    }

    /** 内腔输入损耗抑制组件数量 */
    public int inputLossModules() {
        return data.get(ChishiWirelessTerminalBlockEntity.DATA_INPUT_LOSS);
    }

    /** 内腔输出损耗抑制组件数量 */
    public int outputLossModules() {
        return data.get(ChishiWirelessTerminalBlockEntity.DATA_OUTPUT_LOSS);
    }

    /** 终端短 ID（8 位 hex，与身份卡 ID 同格式） */
    public String getTerminalShortId() {
        return String.format("%08X", data.get(ChishiWirelessTerminalBlockEntity.DATA_TERMINAL_ID));
    }
    ...
    /** 方块实体缺失兜底（客户端空数据构造） */
    public static ChishiWirelessTerminalMenu emptyMenu(int id, Inventory inv) {
        return new ChishiWirelessTerminalMenu(id, inv,
                new SimpleContainerData(ChishiWirelessTerminalBlockEntity.DATA_SLOTS));
    }
