 ...
 */
public class ChishiBodyScannerScreen extends AbstractContainerScreen<ChishiBodyScannerMenu> {
    ...
    public ChishiBodyScannerScreen(ChishiBodyScannerMenu menu, Inventory inv, Component title) {
    ...
    public void render(GuiGraphics gui, int mouseX, int mouseY, float partialTick) {
        ...
        for (int i = 0; i < BodySlot.values().length; i++) {
            ...
            if (!organ.isEmpty()) {
                ...
                gui.drawString(this.font, Component.translatable("gui.template_mod.body_scanner.original"),
        ...
        int statusColor;
        if (total >= 100) {
            statusKey = "gui.template_mod.body_scanner.critical";
            statusColor = 0xFFD64545;
        } else if (total >= 60) {
            statusKey = "gui.template_mod.body_scanner.warning";
            statusColor = 0xFF8B6F1E;
        } else if (total >= 30) {
            statusKey = "gui.template_mod.body_scanner.caution";
            statusColor = 0xFFE0A63A;
        } else {
            statusKey = "gui.template_mod.body_scanner.stable";
            statusColor = 0xFF2E7D32;
        }
        int sumY = this.topPos + 164;
        Component summary = Component.translatable("gui.template_mod.body_scanner.summary", occupied, BodySlot.values().length, total);
