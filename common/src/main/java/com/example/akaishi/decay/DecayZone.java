...
/**
 * 衰竭区域数据：以中心为球心、半径按等级呈 3 的次方区块梯度（3~81 区块 = 48~1296 格）的持续污染区。
 * 区域内：非亡灵实体每 tick 刷新 1 秒"衰变"效果并每秒受魔法伤害；amp≥1 的区域环境方块逐步
 * 腐化为衰竭变体/被破坏（amp=0 泄漏区仅作用于生物）；生物按概率转化（骷髅→凋零骷髅、马→僵尸马/骷髅马、村民→僵尸村民）。
