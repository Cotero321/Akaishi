...
import com.example.akaishi.menu.AkaishiReactorFuelPortMenu;
 ...
 */
public class AkaishiReactorFuelPortBlockEntity extends BlockEntity implements IItemPipeDevice, ExtendedMenuProvider, IDataCarrier {
    ...
    public AkaishiReactorFuelPortBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.CHISHI_REACTOR_FUEL_PORT.get(), pos, state);
        this.buffer = new SimpleContainer(BUFFER_SLOTS) {
            @Override
            public void setChanged() {
                super.setChanged();
                AkaishiReactorFuelPortBlockEntity.this.setChanged();
            }
        };
    }

    public static void serverTick(Level level, BlockPos pos, BlockState state, AkaishiReactorFuelPortBlockEntity be) {
        be.tickServer();
    }

    private void tickServer() {
        AkaishiReactorControllerBlockEntity controller = getController();
    ...
    private AkaishiReactorControllerBlockEntity getController() {
        if (controllerPos == null) {
            return null;
        }
        return level.getBlockEntity(controllerPos) instanceof AkaishiReactorControllerBlockEntity c ? c : null;
    }
    ...
    private void pullEmptyCells(AkaishiReactorControllerBlockEntity controller) {
        for (int i = 0; i < controller.getRodCount(); i++) {
            ItemStack cell = controller.fuelSlots().getItem(i);
            if (cell.getItem() instanceof AkaishiFuelCellItem && AkaishiFuelCellItem.isEmpty(cell)) {
    ...
    private void pushFilledCells(AkaishiReactorControllerBlockEntity controller) {
    ...
    public ItemStack takeCell() {
        ...
        for (int i = 0; i < BUFFER_SLOTS; i++) {
            ...
            if (s.getItem() instanceof AkaishiFuelCellItem && AkaishiFuelCellItem.isEmpty(s)) {
    ...
    public Component getDisplayName() {
        return Component.translatable("block.akaishi.akaishi_reactor_fuel_port");
    }

    @Override
    public AbstractContainerMenu createMenu(int id, Inventory inv, Player player) {
        return new AkaishiReactorFuelPortMenu(id, inv, buffer);
    }
    ...
    public boolean canPlaceItem(int index, ItemStack stack) {
        ...
        return stack.isEmpty() || stack.getItem() instanceof AkaishiFuelCellItem;
    }

    @Override
    public void setItem(int index, ItemStack stack) {
        // 管道灌入不经过类型校验：燃料口只允许空/燃料罐，防止杂物占用缓冲槽卡死物流
        if (stack.isEmpty() || stack.getItem() instanceof AkaishiFuelCellItem) {
