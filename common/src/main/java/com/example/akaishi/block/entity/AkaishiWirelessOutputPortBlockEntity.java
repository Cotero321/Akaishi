...
public class AkaishiWirelessOutputPortBlockEntity extends BlockEntity implements IEnergyProvider, ExtendedMenuProvider, IDataCarrier, IWirelessPortHost {
    ...
    private void tickServer() {
        if (boundCard == null) {
            ...
            // 远程认证：按记录的终端 ID 直达终端，并校验绑定身份在安全表里的「取出」权限
            IWirelessTerminal terminal = WirelessTransferUtil.resolveTerminal(level, boundTerminalId,
                    WirelessFamily.CHISHI, boundCard, AkaishiSecurityPermission.EXTRACT);
            if (terminal != null) {
    ...
    public Component getDisplayName() {
        return Component.translatable("block.akaishi.akaishi_wireless_output_port");
    }
