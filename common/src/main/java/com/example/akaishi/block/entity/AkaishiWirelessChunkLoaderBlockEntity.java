 ...
 */
public class AkaishiWirelessChunkLoaderBlockEntity extends BlockEntity {
    ...
    public AkaishiWirelessChunkLoaderBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.CHISHI_WIRELESS_CHUNK_LOADER.get(), pos, state);
    }

    public static void serverTick(Level level, BlockPos pos, BlockState state, AkaishiWirelessChunkLoaderBlockEntity be) {
        be.tickServer();
    }
    ...
    private BlockPos resolveStructureController(ServerLevel serverLevel) {
        for (BlockPos c : BlockPos.betweenClosed(
            ...
            if (!(serverLevel.getBlockState(c).getBlock() instanceof AkaishiWirelessControllerBlock)) {
                continue;
            }
