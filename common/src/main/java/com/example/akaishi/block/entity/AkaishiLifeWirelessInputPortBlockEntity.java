...
import com.example.akaishi.menu.AkaishiLifeWirelessPortMenu;
...
/**
 * 生命无线输入口方块实体：生命能量管道 → 生命无线网络的发送端（远程设备，无需在外墙上）。
 * 赤能源版 {@link AkaishiWirelessInputPortBlockEntity} 的生命族镜像：纯接收端（仅管道注入，不可抽取）。
 * <p>
 * 手持已授权身份卡右键口即可绑定（绑定卡 UUID 经 NBT 持久化）。每 tick 按卡 + 族（LIFE）反查授权终端：
 * 认证成功则注册上线并推送缓冲生命能量（存入终端绑定储能，损耗按距离计算并消失）；
 * 认证失败/卡被撤销授权则自动下线（每 tick 校验天然支持「撤销即断连」）。
 * 无线传输不限速（无速率上限、无卡档概念）；装配区块加载构架后由终端维持已认证端口区块。
 */
public class AkaishiLifeWirelessInputPortBlockEntity extends BlockEntity
        implements IEnergyProvider, ExtendedMenuProvider, IDataCarrier, IWirelessPortHost {

    // ===== 数据槽（下标与赤版输入口完全一致，端口菜单读取兼容） =====
    ...
    public AkaishiLifeWirelessInputPortBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.CHISHI_LIFE_WIRELESS_INPUT_PORT.get(), pos, state);
        this.buffer = new AkaishiEnergyStorage(LifeEnergyType.INSTANCE, ModConfig.wirelessInputPortBufferCapacity);
    }

    public static void serverTick(Level level, BlockPos pos, BlockState state, AkaishiLifeWirelessInputPortBlockEntity be) {
        be.tickServer();
    }
    ...
    private void tickServer() {
        if (boundCard == null) {
            ...
            // 远程认证：按记录的终端 ID 直达生命族终端，并校验绑定身份在安全表里的「存入」权限
            IWirelessTerminal terminal = WirelessTransferUtil.resolveTerminal(level, boundTerminalId,
                    WirelessFamily.LIFE, boundCard, AkaishiSecurityPermission.INJECT);
            if (terminal != null) {
    ...
    public void bind(UUID card) {
        UUID terminalId = WirelessNetworkManager.findTerminalForIdentity(card, WirelessFamily.LIFE,
    ...
    public Component getDisplayName() {
        return Component.translatable("block.akaishi.akaishi_life_wireless_input_port");
    }

    @Override
    public AbstractContainerMenu createMenu(int id, Inventory inv, Player player) {
        return new AkaishiLifeWirelessPortMenu(id, inv, this);
    }
