...
public class AkaishiEnergyAssemblyBlockEntity extends BlockEntity implements IEnergyProvider, Container, IItemPipeDevice, IDataCarrier {
     ...
     * 采用按能量消耗而非整数 tick：每 tick 产出并消耗 min(剩余, 单 tick 产能速率)，确保每份燃料总产能精确为单台 1.5 倍。
     */
    ...
    public AkaishiEnergyAssemblyBlockEntity(BlockPos pos, BlockState state) {
        ...
        this.inventory = new SimpleContainer(TOTAL_SLOTS) {
            ...
            public void setChanged() {
                super.setChanged();
                AkaishiEnergyAssemblyBlockEntity.this.setChanged();
            }
        };
    ...
    public static double getBoostMultiplier(int upgradeCount) {
        int n = Math.max(0, Math.min(upgradeCount, AkaishiEnergyAssemblyBlockEntity.UPGRADE_SLOTS));
    ...
    /** 服务端 tick：验证结构 + 集中产能 */
    public static void serverTick(Level level, BlockPos pos, BlockState state, AkaishiEnergyAssemblyBlockEntity be) {
        be.tickServer();
    }
    ...
    private void tickServer() {
        ...
        boolean formed = getBlockState().getValue(AkaishiEnergyAssemblyBlock.FORMED);
        ...
        if (formed && energy.getEnergyStored() < MAX_ENERGY) {
            ...
            if (burnEnergy > 0) {
                ...
                int consume = Math.min(ModConfig.energyAssemblyGenerateRate, burnEnergy);
                burnEnergy -= consume;
    ...
    private void setFormed(boolean formed) {
        level.setBlock(worldPosition, getBlockState().setValue(AkaishiEnergyAssemblyBlock.FORMED, formed), 3);
