...
public class AkaishiAdvancedPurifierBlockEntity extends BlockEntity implements IEnergyProvider, Container, IItemPipeDevice, IDataCarrier {
    ...
    private AkaishiPurifierBlockEntity cachedCenter;

    public AkaishiAdvancedPurifierBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.CHISHI_ADVANCED_PURIFIER.get(), pos, state);
        this.energy = new AkaishiEnergyStorage(AkaishiEnergyType.INSTANCE, 10000);
        this.inventory = new SimpleContainer(2) {
            @Override
            public void setChanged() {
                super.setChanged();
                AkaishiAdvancedPurifierBlockEntity.this.setChanged();
            }
        };
    }
        ...
        for (int dx = -1; dx <= 1; dx++) {
            for (int dy = -1; dy <= 1; dy++) {
                for (int dz = -1; dz <= 1; dz++) {
                    ...
                    if (level.getBlockEntity(p) instanceof AkaishiAdvancedPurifierBlockEntity shell) {
                        shell.markDirty();
                    }
                }
            }
    ...
    public static void serverTick(Level level, BlockPos pos, BlockState state, AkaishiAdvancedPurifierBlockEntity be) {
        be.tickServer();
    }
    ...
    private void refreshCache() {
        ...
        if (formed != blockState.getValue(AkaishiAdvancedPurifierBlock.FORMED)) {
            level.setBlock(worldPosition, blockState.setValue(AkaishiAdvancedPurifierBlock.FORMED, formed), 3);
            setChanged();
        }
    }
    ...
    public AkaishiPurifierBlockEntity findMatrixCenter() {
        for (int dx = -1; dx <= 1; dx++) {
            for (int dy = -1; dy <= 1; dy++) {
                for (int dz = -1; dz <= 1; dz++) {
                    if (dx == 0 && dy == 0 && dz == 0) {
                    ...
                    if (s.getBlock() instanceof AkaishiPurifierBlock
                            && s.getValue(AkaishiPurifierBlock.FORMED)) {
                        if (level.getBlockEntity(p) instanceof AkaishiPurifierBlockEntity center && center.isMatrixFormed()) {
                            return center;
                        }
                    }
                }
