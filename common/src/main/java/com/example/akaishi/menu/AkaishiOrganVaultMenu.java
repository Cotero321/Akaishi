 ...
 * 选页纯客户端本地状态（AkaishiOrganVaultScreen 切换），无需网络包。
 */
public class AkaishiOrganVaultMenu extends AbstractContainerMenu {

    /** 当前选中页（客户端本地状态，服务端无意义） */
    private int currentPage;
    @Nullable
    private final AkaishiOrganVaultBlockEntity vault;
    private final ContainerData data;

    public AkaishiOrganVaultMenu(int id, Inventory inv, AkaishiOrganVaultBlockEntity vault) {
    ...
    public AkaishiOrganVaultMenu(int id, Inventory inv, Container container, ContainerData data, BlockPos pos) {
        super(ModMenus.CHISHI_ORGAN_VAULT.get(), id);
        this.vault = null;
        this.data = data;
        for (int i = 0; i < AkaishiOrganVaultBlockEntity.PER_PAGE; i++) {
        ...
        for (int i = 0; i < AkaishiOrganVaultBlockEntity.TEMP_SIZE; i++) {
            addSlot(new Slot(container, AkaishiOrganVaultBlockEntity.TEMP_START + i,
                    124 + (i % 3) * 18, 22 + (i / 3) * 18));
        }
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 9; col++) {
    ...
    public void setCurrentPage(int page) {
        this.currentPage = Math.max(0, Math.min(AkaishiOrganVaultBlockEntity.PAGE_COUNT - 1, page));
        // 页槽映射随页变化，直接重绘
        for (int i = 0; i < AkaishiOrganVaultBlockEntity.PER_PAGE; i++) {
    ...
    public boolean isActive() {
        return data.get(AkaishiOrganVaultBlockEntity.DATA_ACTIVE) > 0;
    }
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        if (slot != null && slot.hasItem()) {
            ...
            result = current.copy();
            int machineEnd = AkaishiOrganVaultBlockEntity.PER_PAGE + AkaishiOrganVaultBlockEntity.TEMP_SIZE;
            if (index < machineEnd) {
                // 库内 → 背包
                if (!this.moveItemStackTo(current, machineEnd, machineEnd + 36, true)) {
                    return ItemStack.EMPTY;
                }
            } else if (current.getItem() instanceof AkaishiOrganItem) {
    ...
    static class VaultSlot extends Slot {
        ...
        VaultSlot(AkaishiOrganVaultBlockEntity vault, int page, int indexInPage, int x, int y) {
        ...
        private int actualIndex() {
            if (page < 0) {
                return AkaishiOrganVaultBlockEntity.TEMP_START + indexInPage;
            }
            return page * AkaishiOrganVaultBlockEntity.PER_PAGE + indexInPage;
        }
    }
}
