...
/**
 * 生命储存串联器界面：横向生命能量条 + 单位缩写数值 + 结构状态提示。
 * 布局与赤能源储存串联器一致（复用同一张基础贴图，生命条为绿色），提示文案走生命族键。
 */
public class AkaishiLifeEnergyCellSerializerScreen extends AbstractContainerScreen<AkaishiLifeEnergyCellSerializerMenu> {
    ...
    public AkaishiLifeEnergyCellSerializerScreen(AkaishiLifeEnergyCellSerializerMenu menu, Inventory inv, Component title) {
    ...
    public void render(GuiGraphics gui, int mouseX, int mouseY, float partialTick) {
        ...
        Component hint = menu.isFormed()
                ? Component.translatable("gui.akaishi.life_serializer.formed", formatEnergy(menu.getMaxEnergy()))
                : Component.translatable("gui.akaishi.life_serializer.unformed");
        ...
        if (isHovering(BAR_X, BAR_Y, BAR_W, BAR_H, mouseX, mouseY)) {
            gui.renderTooltip(this.font,
                    Component.translatable("gui.akaishi.life", formatEnergy(menu.getEnergy()), formatEnergy(menu.getMaxEnergy())),
                    mouseX, mouseY);
        }
    }
}
