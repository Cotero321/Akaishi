...
public class AkaishiReactorControllerMenu extends AbstractContainerMenu {
    ...
    public AkaishiReactorControllerMenu(int id, Inventory inv, AkaishiReactorControllerBlockEntity be) {
        this(id, inv, be.fuelSlots(), be.data(), resolveCoolers(be));
    }

    /** 客户端/异常兜底：无结构信息，散热片槽位使用空容器占位 */
    public AkaishiReactorControllerMenu(int id, Inventory inv, Container container, ContainerData data) {
        this(id, inv, container, data, null);
    }

    private AkaishiReactorControllerMenu(int id, Inventory inv, Container container, ContainerData data,
                                        List<AkaishiReactorCoolerBlockEntity> coolers) {
        ...
        for (int row = 0; row < 2; row++) {
            for (int col = 0; col < 5; col++) {
                ...
                addSlot(new FuelCellSlot(container, idx,
                        44 + col * 18, 58 + row * 18,
                        () -> this.page != 0 || idx >= this.data.get(AkaishiReactorControllerBlockEntity.DATA_ROD_COUNT)));
            }
        }
        ...
        for (int i = 0; i < COOLER_SLOT_COUNT; i++) {
            ...
            addSlot(new HeatSinkSlot(c, AkaishiReactorCoolerBlockEntity.SINK_SLOT,
    ...
    private static List<AkaishiReactorCoolerBlockEntity> resolveCoolers(AkaishiReactorControllerBlockEntity be) {
        List<AkaishiReactorCoolerBlockEntity> out = new ArrayList<>();
        ReactorStructure.Result s = be.getStructure();
        if (s == null || be.getLevel() == null) {
            return out;
        }
        for (BlockPos p : s.coolers) {
            if (be.getLevel().getBlockEntity(p) instanceof AkaishiReactorCoolerBlockEntity c) {
                out.add(c);
            }
        }
        return out;
    ...
    public int getTemp() {
        return data.get(AkaishiReactorControllerBlockEntity.DATA_TEMP);
    }

    public boolean isFormed() {
        return data.get(AkaishiReactorControllerBlockEntity.DATA_FORMED) == 1;
    }

    public int getRodCount() {
        return data.get(AkaishiReactorControllerBlockEntity.DATA_ROD_COUNT);
    }

    public int getEffectiveCoolers() {
        return data.get(AkaishiReactorControllerBlockEntity.DATA_EFFECTIVE_COOLERS);
    }

    public int getCoolingPercent() {
        return data.get(AkaishiReactorControllerBlockEntity.DATA_COOLING_PERCENT);
    }

    public int getCoolerDurability() {
        return data.get(AkaishiReactorControllerBlockEntity.DATA_COOLER_DURABILITY);
    }

    /** 结构内散热组件总数（客户端绘制散热片槽位用） */
    public int getCoolerCount() {
        return data.get(AkaishiReactorControllerBlockEntity.DATA_COOLER_COUNT);
    }
    ...
    public boolean isWarning() {
        return data.get(AkaishiReactorControllerBlockEntity.DATA_WARNING) == 1;
    }

    public int getActiveSlots() {
        return data.get(AkaishiReactorControllerBlockEntity.DATA_ACTIVE_SLOTS);
    }
    ...
    public double getFuelDrainPerTick() {
        return data.get(AkaishiReactorControllerBlockEntity.DATA_FUEL_DRAIN) / 1000.0;
    }

    public boolean isWasteFull() {
        return data.get(AkaishiReactorControllerBlockEntity.DATA_WASTE_FULL) == 1;
    }

    /** 废品罐内衰竭燃料种类数（tooltip 展示） */
    public int getWasteTypes() {
        return data.get(AkaishiReactorControllerBlockEntity.DATA_WASTE_TYPES);
    }
