 ...
 */
public class AkaishiEquipmentForgerScreen extends AbstractContainerScreen<AkaishiEquipmentForgerMenu> {
    ...
    private boolean showUpgradeButton(int i) {
        return i != AkaishiUpgradeHelper.UpgradeType.EFFICIENCY.ordinal()
                || AkaishiUpgradeHelper.isEfficiencyTool(menu.getGearItem());
    }

    public AkaishiEquipmentForgerScreen(AkaishiEquipmentForgerMenu menu, Inventory inv, Component title) {
    ...
    protected void renderBg(GuiGraphics gui, float partialTick, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < AkaishiUpgradeHelper.UpgradeType.values().length; i++) {
            ...
            int bx = x + attrBtnX(i);
            boolean enabled = menu.getUpgradePoints() > 0 && menu.getBaseCount(i) < AkaishiUpgradeHelper.FORGE_UPGRADE_POINTS;
    ...
    protected void renderLabels(GuiGraphics gui, int mouseX, int mouseY) {
        ...
        gui.drawString(this.font,
                Component.translatable("gui.akaishi.forger.points", menu.getUpgradePoints()),
                20, 48, menu.getUpgradePoints() > 0 ? 0xFFFFD030 : 0xE0E0E0, false);

        // 属性按钮文字（按钮 key 与 UpgradeType 顺序一致，效率仅挖掘工具显示）
        AkaishiUpgradeHelper.UpgradeType[] types = AkaishiUpgradeHelper.UpgradeType.values();
        ...
        gui.drawCenteredString(this.font, Component.translatable("gui.akaishi.forger.forge"),
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        int btnCount = AkaishiUpgradeHelper.UpgradeType.values().length;
        if (button == 0 || button == 1) {
            for (int i = 0; i < btnCount; i++) {
                ...
                if (mouseX >= bx && mouseX < bx + ATTR_BTN_W
                    ...
                    int id = hasShiftDown() || button == 1
                            ? AkaishiEquipmentForgerMenu.BTN_REMOVE_OFFSET + i
                            : AkaishiEquipmentForgerMenu.BTN_ADD_BASE + i;
            ...
            if (mouseX >= this.leftPos + FORGE_BTN_X && mouseX < this.leftPos + FORGE_BTN_X + FORGE_BTN_W
                ...
                this.minecraft.gameMode.handleInventoryButtonClick(this.menu.containerId, AkaishiEquipmentForgerMenu.BUTTON_FORGE);
                return true;
            }
        }
