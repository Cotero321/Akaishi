...
/**
 * 赤能源发生机界面（176×198 自绘，与单槽机器风格一致，不再依赖贴图）：
 * 燃料槽 + 单个升级组件槽（右上角）+ 赤能源条 + 燃烧进度条 + 加速倍率。
 * 坐标全部由代码决定，并与 {@link AkaishiEnergyGeneratorMenu} 的槽位一致。
 */
public class AkaishiEnergyGeneratorScreen extends AbstractContainerScreen<AkaishiEnergyGeneratorMenu> {
    ...
    private static final int BOOST_X = 20, BOOST_Y = 90;
    ...
    protected void renderLabels(GuiGraphics gui, int mouseX, int mouseY) {
        ...
        gui.drawString(this.font, Component.translatable("gui.akaishi.upgrade.tag"),
                UPGRADE_X - 36, UPGRADE_Y + 4, 0xFF707070, false);
    ...
    protected void renderTooltip(GuiGraphics gui, int mouseX, int mouseY) {
        ...
        if (isHovering(BAR_X, ENERGY_Y, BAR_W, BAR_H, mouseX, mouseY)) {
            gui.renderTooltip(this.font, Component.translatable("gui.akaishi.energy",
                    EnergyFormat.format(menu.getEnergy()), EnergyFormat.format(AkaishiEnergyGeneratorBlockEntity.MAX_ENERGY)),
                    mouseX, mouseY);
            ...
            gui.renderTooltip(this.font,
                    Component.translatable("gui.akaishi.energy_generator.fuel_slot"), mouseX, mouseY);
