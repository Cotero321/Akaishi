package com.example.akaishi.menu;

import com.example.akaishi.item.AkaishiFuelCellItem;
import net.minecraft.world.Container;
import net.minecraft.world.inventory.Slot;
import net.minecraft.world.item.ItemStack;

import java.util.function.BooleanSupplier;

/**
 * 燃料单元槽位：仅允许放入 {@link AkaishiFuelCellItem}，且通过 hidden 供应商实现
 * 燃料页专属激活（切到其它页签时隐藏、不可交互）。
 */
public class FuelCellSlot extends Slot {

    private final BooleanSupplier hidden;

    public FuelCellSlot(Container container, int slot, int x, int y, BooleanSupplier hidden) {
        super(container, slot, x, y);
        this.hidden = hidden;
    }

    @Override
    public boolean mayPlace(ItemStack stack) {
        return stack.getItem() instanceof AkaishiFuelCellItem;
    }

    @Override
    public boolean isActive() {
        return !hidden.getAsBoolean() && super.isActive();
    }
}
