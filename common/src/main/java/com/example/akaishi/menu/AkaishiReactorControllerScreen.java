 ...
 * </pre>
 */
public class AkaishiReactorControllerScreen extends AbstractContainerScreen<AkaishiReactorControllerMenu> {

    private static final ResourceLocation TEXTURE =
            new ResourceLocation(AkaishiMod.MOD_ID, "textures/gui/akaishi_reactor_controller.png");
    ...
    public AkaishiReactorControllerScreen(AkaishiReactorControllerMenu menu, Inventory inv, Component title) {
    ...
    private void drawTabs(GuiGraphics gui, int x, int y, int mouseX, int mouseY) {
        String[] keys = {"gui.akaishi.reactor.tab_fuel",
                "gui.akaishi.reactor.tab_temp",
                "gui.akaishi.reactor.tab_status"};
    ...
    protected void renderLabels(GuiGraphics gui, int mouseX, int mouseY) {
        ...
        AkaishiReactorControllerMenu m = this.menu;
        switch (m.getPage()) {
    ...
    private void drawFuelLabels(GuiGraphics gui, AkaishiReactorControllerMenu m) {
        gui.drawString(this.font,
                Component.translatable(m.isFormed() ? "gui.akaishi.reactor.formed"
                        : "gui.akaishi.reactor.unformed"),
                8, 42, m.isFormed() ? OK : WARN, false);
        gui.drawString(this.font, Component.translatable("gui.akaishi.reactor.fuel_used",
                usedFuelSlots(), AkaishiReactorControllerMenu.FUEL_SLOT_COUNT), 8, 98, TEXT_DIM, false);
    }

    /** 温度页：右侧温度/散热/有效/耐久读数（文本右移，避开 5×4 槽位区） */
    private void drawTempLabels(GuiGraphics gui, AkaishiReactorControllerMenu m) {
        int t = clampTemp(m.getTemp());
        gui.drawString(this.font, Component.translatable("gui.akaishi.reactor.temp_label"),
        ...
        gui.drawString(this.font, Component.translatable("gui.akaishi.reactor.cooling",
                m.getCoolingPercent()), TEMP_TEXT_X, TEMP_TEXT_Y0 + TEMP_TEXT_STEP * 2, TEXT, false);
        gui.drawString(this.font, Component.translatable("gui.akaishi.reactor.coolers",
                m.getEffectiveCoolers()), TEMP_TEXT_X, TEMP_TEXT_Y0 + TEMP_TEXT_STEP * 3, TEXT, false);
        int dur = m.getCoolerDurability();
        gui.drawString(this.font, Component.translatable("gui.akaishi.reactor.cooler_durability", dur),
                TEMP_TEXT_X, TEMP_TEXT_Y0 + TEMP_TEXT_STEP * 4, dur <= 25 ? BAD : TEXT, false);
    }

    /** 状态页：运行状态（高温警告中产量降低）+ 废品储量（图二内容归入本页） */
    private void drawStatusLabels(GuiGraphics gui, AkaishiReactorControllerMenu m) {
        boolean warn = m.isWarning();
        gui.drawString(this.font, Component.translatable(warn
                ? "gui.akaishi.reactor.warning.on" : "gui.akaishi.reactor.warning.off"),
                8, 40, warn ? BAD : OK, false);
        gui.drawString(this.font, Component.translatable("gui.akaishi.reactor.energy_rate",
                EnergyFormat.format(m.getEnergyPerTick())), 8, 52, TEXT, false);
        gui.drawString(this.font, Component.translatable("gui.akaishi.reactor.waste_label"),
                WASTE_X, 68, TEXT_DIM, false);
        gui.drawString(this.font, Component.translatable(m.isWasteFull()
                ? "gui.akaishi.reactor.waste_need_clear" : "gui.akaishi.reactor.waste_ok"),
    ...
    private int usedFuelSlots() {
        int n = 0;
        for (int i = 0; i < AkaishiReactorControllerMenu.FUEL_SLOT_COUNT; i++) {
