...
/**
 * 无线生命便捷终端界面（手持物品，只读遥控面板，赤版便携终端的生命镜像）。
 * 两页互斥切换：运行情况（身份卡/认证终端/口统计）+ 能量储存情况（储能条，生命绿）。
 * 数据由服务端 broadcastChanges 每 tick 按生命族扫背包身份卡刷新。
 * 198 高布局：标题 y=6 与切页按钮 y=16 错开，内容区 y=36 起舒展排布。
 */
public class AkaishiLifeWirelessPortableTerminalScreen extends AbstractContainerScreen<AkaishiLifeWirelessPortableTerminalMenu> {
    ...
    public AkaishiLifeWirelessPortableTerminalScreen(AkaishiLifeWirelessPortableTerminalMenu menu, Inventory inv, Component title) {
    ...
    protected void renderLabels(GuiGraphics gui, int mouseX, int mouseY) {
        ...
        drawTabLabel(gui, 88, "gui.akaishi.wireless.tab.energy", currentPage == 1);
    ...
    private void renderRunPage(GuiGraphics gui) {
        ...
        gui.drawString(this.font, linked
                ? Component.translatable("gui.akaishi.wireless.port.authed",
                        String.format("%08X", menu.getTerminalHash()))
                : Component.translatable("gui.akaishi.wireless.port.no_terminal"),
