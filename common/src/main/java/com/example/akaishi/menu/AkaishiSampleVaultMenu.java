 ...
 */
public class AkaishiSampleVaultMenu extends AbstractContainerMenu {

    private final Container container;

    public AkaishiSampleVaultMenu(int id, Inventory inv, AkaishiSampleVaultBlockEntity vault) {
        this(id, inv, vault.inventory());
    }

    /** 完整构造（网络工厂共用） */
    public AkaishiSampleVaultMenu(int id, Inventory inv, Container container) {
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        if (slot != null && slot.hasItem()) {
            ...
            result = current.copy();
            if (index < AkaishiSampleVaultBlockEntity.SAMPLE_SLOTS) {
                // 库内 → 背包
                if (!this.moveItemStackTo(current, AkaishiSampleVaultBlockEntity.SAMPLE_SLOTS,
                        AkaishiSampleVaultBlockEntity.SAMPLE_SLOTS + 36, true)) {
                    return ItemStack.EMPTY;
                }
            } else if (current.getItem() instanceof AkaishiLifeSampleItem) {
                // 背包 → 库内（同 NBT 自动合并，BE tick 再兜底）
                if (!this.moveItemStackTo(current, 0, AkaishiSampleVaultBlockEntity.SAMPLE_SLOTS, false)) {
                    return ItemStack.EMPTY;
                }
            } else {
                return ItemStack.EMPTY;
