...
public class AkaishiEnergyLiquefierMenu extends AbstractContainerMenu {
    ...
    public AkaishiEnergyLiquefierMenu(int id, Inventory inv, AkaishiEnergyLiquefierBlockEntity be) {
        this(id, inv, be.inventory(), be.data(), be.getUpgradeSlots());
    }

    public AkaishiEnergyLiquefierMenu(int id, Inventory inv, Container container, ContainerData data, Container upgrades) {
    ...
    public int getProgress() {
        return data.get(AkaishiEnergyLiquefierBlockEntity.DATA_PROGRESS);
    }
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        if (slot != null && slot.hasItem()) {
            ...
            if (index < MACHINE_SLOT_END) {
                ...
                // 玩家背包：升级组件自动按槽位类型过滤入升级槽，材料/固态物入对应输入槽
