 ...
 * 数据来自 {@link AkaishiGeneAnalyzerMenu} 的 ContainerData。
 */
public class AkaishiGeneAnalyzerScreen extends AbstractContainerScreen<AkaishiGeneAnalyzerMenu> {
    ...
    protected void renderBg(GuiGraphics gui, float partialTick, int mouseX, int mouseY) {
        ...
        // 机器槽位框（贴图无图形，自绘补齐；0/1=升级槽 2/3=输入/输出槽）
    ...
    public void render(GuiGraphics gui, int mouseX, int mouseY, float partialTick) {
        ...
        if (isHovering(PROGRESS_X, PROGRESS_Y, PROGRESS_W, PROGRESS_H, mouseX, mouseY)) {
            gui.renderTooltip(this.font,
                    Component.translatable("gui.akaishi.gene_analyzer.progress",
